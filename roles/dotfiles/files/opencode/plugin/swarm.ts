...
// OpenChamber's hardened-runtime sandbox blocks ALL process spawning from plugins:
//   - Bun.spawn → ENOENT (posix_spawn blocked)
//   - child_process.spawn → exit code -2 (process killed)
//   - BunShell (input.$) → ENOENT on everything, even echo
// The ONLY way to execute commands is through the PTY API (POST /pty) via the
// OpenCode SDK client (input.client.pty.create). The OpenCode SERVER creates the
// process, not the plugin. Output goes to a temp file that the plugin reads with fs.
...
// OpenChamber sandbox blocks direct spawn of Homebrew binaries (exit code -2).
// Use /bin/bash -c as intermediary — the bash tool already proves this path works.
// JSON args are single-quote-escaped (' → '\'') to survive shell parsing.
const SWARM_BINARY = isWindows ? "swarm.cmd" : "/home/me/.config/opencode/node_modules/.bin/swarm";
const SWARM_CLI = isWindows ? "swarm.cmd" : "/bin/bash";
const OPENCODE_CLI = isWindows ? "opencode.cmd" : "/home/me/.nix-profile/bin/opencode";
 ...
 *
 * OpenChamber's hardened-runtime sandbox blocks ALL process spawning from plugins
 * (Bun.spawn, child_process.spawn, BunShell). The ONLY workaround is the PTY API:
 * `input.client.pty.create()` creates a pseudo-terminal through the OpenCode SERVER
 * (which CAN spawn processes). The command output is redirected to a temp file that
 * this function reads with fs.readFileSync — file I/O is NOT blocked.
 *
...
// NOTE: swarm 0.63.2 removed the `swarm_complete_subtask` CLI tool (it was an
// in-process-only helper in the full npm plugin). Workers complete subtasks via
// the working `swarm_complete` tool (used by all prompts), so this dead wrapper
// was removed to avoid UNKNOWN_TOOL errors.
...
const swarm_get_strategy_insights = tool({
  ...
  // 0.63.2: per-name insight CLI tools were consolidated into `swarm_insights`
  // (metrics: success_rate / avg_duration). Route there — it tolerates extra
  // args and returns {insights:[...]} — so this never throws UNKNOWN_TOOL.
  execute: (args, ctx) => execTool("swarm_insights", { metrics: ["success_rate", "avg_duration"], ...args }, ctx),
});
...
const swarm_get_file_insights = tool({
  ...
  // 0.63.2: consolidated into `swarm_insights` (see swarm_get_strategy_insights).
  execute: (args, ctx) => execTool("swarm_insights", { metrics: ["success_rate", "avg_duration"], ...args }, ctx),
});
...
const swarm_get_pattern_insights = tool({
  ...
  args: {},
  // 0.63.2: consolidated into `swarm_insights` (see swarm_get_strategy_insights).
  execute: (args, ctx) => execTool("swarm_insights", { metrics: ["success_rate", "avg_duration"] }, ctx),
});
...
/**
 * Spawn a binary through Apple-signed /bin/bash so OpenChamber's hardened-runtime
 * sandbox doesn't ENOENT (exit -2) on Homebrew bun-shebang scripts (e.g. `swarm`).
 * Uses Bun.spawn (the OpenCode plugin sandbox blocks Node's child_process.spawn).
 * Args go through bash `exec "$0" "$@"` so they need NO shell escaping and cannot
 * inject — safe even for arbitrary prompt text. Returns a Node-spawn-like shape.
 */
async function bashSpawn(
  binary: string,
  args: string[],
  ...
  const cmd = isWindows
    ? [binary, ...args]
    : ["/bin/bash", "-c", 'exec "$0" "$@"', binary, ...args];
  const proc = Bun.spawn(cmd, {
    cwd: opts.cwd ?? projectDirectory,
    stdin: null,
    stdout: "pipe",
    stderr: "pipe",
    env: {
...
async function querySwarmState(sessionID: string): Promise<SwarmStateSnapshot> {
  ...
  try {
    // Query cells via swarm CLI (through /bin/bash so OpenChamber's sandbox
    // doesn't ENOENT on the bun-shebang swarm script).
    const cliStart = Date.now();
    const cellsResult = await bashSpawn(SWARM_BINARY, ["tool", "hive_query"]);
...
async function generateCompactionPrompt(
  ...
  const startTime = Date.now();
  // Used only to regenerate an orchestration-recovery prompt after compaction.
  // GUI (OpenChamber) has no shell env; keep this on the cheap workhorse.
  // Override with OPENCODE_LITE_MODEL in the TUI if desired.
  const liteModel = process.env.OPENCODE_LITE_MODEL || "deepseek/deepseek-v4-flash";
...
Keep the prompt concise but actionable. Use actual data from the snapshot, not placeholders. Include the ASCII header and ALL coordinator mandates.`;
    ...
    const result = await bashSpawn(OPENCODE_CLI, ["run", "-m", liteModel, "--", promptText], { timeoutMs: 30000 });
...
async function detectSwarm(): Promise<SwarmDetection> {
  ...
  try {
    const cliStart = Date.now();
    const result = await bashSpawn(SWARM_BINARY, ["tool", "hive_query"]);
    const cliDuration = Date.now() - cliStart;
...
const SwarmPlugin: Plugin = async (
  ...
  // PTY API (input.client.pty.create) is used for all command execution —
  // this is the ONLY spawn mechanism that works in the OpenChamber sandbox.
